...
├── nginx/Dockerfile                # Imagen todosevilla-proxy (nginx + OpenSSL)
├── nginx/proxy-entrypoint.sh       # Genera certs autofirmados en el arranque
...
15. **Healthcheck del `app` con `node`, no con wget de BusyBox**: la imagen es Alpine y su wget (BusyBox) no soporta todos los flags (`--no-verbose`, combinaciones de `-q`/`--spider`), lo que marcaba el contenedor "unhealthy" de forma falsa (exit 137 = matado por Swarm). Usar `["CMD","node","-e","const http=require('http');http.get('http://localhost:3000/api/health',r=>process.exit(r.statusCode===200?0:1)).on('error',()=>process.exit(1)).setTimeout(4000,()=>process.exit(1))"]`. No volver a un healthcheck basado en `wget` sobre node:alpine sin verificarlo antes.
16. **Runner standalone y CLI de Prisma**: el `entrypoint.sh` invoca `node ./node_modules/prisma/build/index.js migrate deploy`. El CLI necesita TODAS sus dependencias transitivas (`@prisma/internals`, `@prisma/config`, `yaml`, etc.), que el standalone de Next.js NO traza. Por eso en el Dockerfile se copia `node_modules` **completo** desde la etapa `deps` (que además incluye engines nativos y bcrypt para el seed). No volver al cherry-pick de `node_modules/prisma` + `@prisma`, o las migraciones fallan con `Cannot find module '@prisma/internals'` → exit(1).
...
19. **Credenciales de Postgres y volumen `pgdata`**: el contenedor oficial de Postgres solo aplica `POSTGRES_USER`/`POSTGRES_PASSWORD` la **primera vez** que se inicializa el volumen. Si el volumen se creó con otros valores, cambiar las variables en el compose NO cambia la contraseña almacenada → la app falla con `Authentication failed ... are not valid`. Si no hay datos que preservar, recrear el volumen (`docker stack rm todosevilla` + `docker volume rm todosevilla_pgdata` + redeploy). Diagnosticar con: `docker exec <db> sh -c 'PGPASSWORD=<pass> psql -h 127.0.0.1 -U <user> -d <db> -tAc "select 1"'`.